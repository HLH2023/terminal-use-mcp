...
> 最后更新: 2026-06-14
> V1 状态: PHASE 0-15 ALL COMPLETE — 生产就绪 ✅
> V2 状态: V2-0 至 V2-5 全部完成 ✅
...
4. **v1 平台范围**: Linux ✅ / macOS ✅ (best-effort) / WSL2 ✅ (best-effort) /
   Native Windows ❌ (v2 ConPTY)
...
## V2 远程终端控制 (Remote Terminal Control over SSH)
...
- `docs/V2_REMOTE_TERMINAL_GUIDE.md` ✅ (完整 28 项标准 + 19 章节 + 实施指导)
...
| Phase | 内容 | 依赖 | 状态 |
|-------|------|------|------|
| V2-0 | 远程设计落文档 (DEV-PLAN/PROGRESS/README/SKILL.md) | V1 complete | 🔄 进行中 |
| V2-1 | TerminalTarget + SshHostProfile + hosts.json loader + RemoteCwdPolicy + tests | V2-0 | ✅ 完成 |
| V2-2 | known_hosts/pinned fingerprint + ssh-agent auth + key-file + verify_target + tests | V2-1 | ✅ 完成（含 MCP 注册接线） |
| V2-3 | ssh-pty Provider (ssh2 channel + xterm adapter + I/O + resize + kill + transcript) | V2-2 | ✅ 完成 |
| V2-4 | ssh-tmux Provider (remote tmux commands + attach/list/snapshot/type/press/kill) | V2-2 | ✅ 完成 |
| V2-5 | Remote examples + troubleshooting + integration evidence + self-critique | V2-3+V2-4 | ⬜ 未开始 |

### V2 完成标准 (28 项)

| # | 标准 | 状态 |
|---|------|------|
| 1 | `TerminalTarget` 已实现 | ✅ V2-1 |
| 2 | SSH profile loader 已实现 | ✅ V2-1 |
| 3 | known_hosts 或 pinned fingerprint 校验已实现 | ✅ V2-2 单元测试通过 |
| 4 | 默认禁止 inline SSH target | ✅ V2-1 |
| 5 | 默认禁止密码登录 | ✅ V2-1 |
| 6 | `ssh-pty` Provider 可用 | ✅ V2-3 单元测试 + 合约骨架 |
| 7 | `ssh-tmux` Provider 可用 | ✅ V2-4 单元测试 + 合约骨架 |
...
| 11 | `terminal.start` 支持 `target.kind=ssh` | ✅ V2-3 existing start tool 已透传 target/provider |
| 12 | `terminal.snapshot/wait/type/press/paste/resize/kill/export_transcript` 支持远程 session | ✅ V2-3/V2-4 provider 层完成 |
| 13 | 远程 session artifact 不包含敏感信息 | ✅ V2-3 metadata 不含私钥/passphrase/token/env 值 |
| 14 | RemoteCwdPolicy 有测试 | ✅ V2-1 |
| 15 | host key mismatch 有测试 | ✅ V2-3 pinned + known_hosts 握手 key mismatch 单元覆盖 |
| 16 | auth failure 有测试 | ✅ key-file/agent 本地 preflight 失败覆盖 |
| 17 | connection timeout 有测试 | ⬜ |
| 18 | remote fixture 集成测试通过，或在当前环境明确 skip 并给出原因 | ✅ V2-3/V2-4 contract tests skip: No SSH fixture available |
| 19 | 至少一个真实远程联调示例文档完成 | ⬜ |
| 20 | SKILL.md 已补充远程规则 | ⬜ |
| 21 | README 已补充远程章节 | ⬜ |
...
| `src/targets/ssh-host-config.ts` | ✅ 补全 | async hosts.json loader；支持默认路径、`TERMINAL_USE_HOSTS_CONFIG`、`~` 展开、cache、缺失文件空 map、JSON/profile/auth 校验；保留 `expandUserPath()` 兼容 V2-2 |
| `src/targets/ssh-profile-loader.ts` | ✅ 补全 | 新增 `resolveSshTarget()`；默认拒绝 inline SSH；profile + inline override 合并；保留 `getSshProfile()` 兼容 V2-2 |
...
| `src/terminal/errors.ts` | ✅ 扩展 | 新增 10 个 V2 SSH/Remote 错误码与错误类 |
...
| `tests/unit/session-manager.test.ts` | ✅ 更新 | 旧 mock config 补齐 V2 新配置字段，保持 LSP clean |
...
1. V2-2: 在当前 V2-2 预检代码基础上完成 `hostsConfig` 注入与 MCP tools 接线。
2. V2-3: 在 V2-2 接线完成后实现 `ssh-pty` Provider，不在 V2-1 中提前引入 `ssh2`。
3. V2-4: 在 V2-2 接线完成后实现 `ssh-tmux` Provider。

### V2-2 实现记录（2026-06-13）
...
| `src/targets/target-types.ts` | ✅ V2-1 已补全 | V2-2 依赖其中 `SshAuthRef` / `SshHostProfile` 类型 |
| `src/targets/ssh-host-config.ts` | ✅ V2-1 已补全 | V2-2 复用其中 `expandUserPath()` 路径展开兼容入口 |
| `src/targets/ssh-profile-loader.ts` | ✅ V2-1 已补全 | V2-2 复用其中 `getSshProfile()` profile 查询入口 |
...
备注：一次 `tests/unit` 目录级 LSP 扫描发现未改文件 `tests/unit/session-manager.test.ts` 中与 V2-1 配置类型相关的 diagnostic；本会话新增测试文件逐个扫描均为 0 diagnostics，且 typecheck/full test 均通过。

#### 技术笔记

- `verify_target` 在 V2-2 阶段不执行真实 SSH 连接；只验证 profile、known_hosts/pinned fingerprint 格式和本地认证材料可访问性，避免提前引入 `ssh2`。
- `known_hosts` 的 `@revoked` 条目被跳过，不能作为可信 host key；hashed host pattern 暂不反解，后续可在 V2-3 连接阶段结合实际 host key 策略扩展。
- `ssh-auth` 对 key-file 只做 `fs.access(..., R_OK)`，不会读取私钥内容；对 `passphraseEnv` 只检查环境变量名是否存在，不读取变量值。
- 三个 V2 target tool 注册函数暂未接入 `createMcpServer()`，因为当前 V1 MCP smoke 测试固定断言 22 tools，且 `hostsConfig` 注入由 V2-1 集成；后续接线时需同步更新 MCP tool count 至 25。

#### 下一步待办

1. V2-1 合并完整 `target-types.ts` / `ssh-host-config.ts` / `ssh-profile-loader.ts` 后，复核本会话最小占位是否需要删除或合并。
...
3. V2-3 建立真实 SSH 握手后，用实际 host key 与本会话返回的 known_hosts fingerprint 做最终 mismatch 校验。

### V2-3 实现记录（2026-06-13） ✅
...
| `src/providers/provider.ts` | ✅ 更新 | `StartInput.target` 与 `TerminalSession.metadata` 透传 V2 SSH target/session metadata |
| `src/providers/provider-registry.ts` | ✅ 更新 | 注册 `SshPtyProvider`，不影响 V1 provider |
...
| `src/tools/start.ts` | ✅ 更新 | 既有 `terminal.start` schema 支持 `ssh-pty` / `ssh-tmux` 与 `target` 字段；未注册新的 V2 tools |
...
- 当前无真实 SSH fixture，contract test 明确 skip；后续 V2-5 需补 Docker SSH fixture 或手动真实主机证据。

#### 下一步待办

1. V2-3 follow-up：补 Docker SSH fixture 后启用 `ssh-pty-contract.test.ts`，覆盖真实 start/wait/snapshot/input/resize/transcript/kill。
2. V2-2/V2 tools 接线：在 `createMcpServer()` 注入 `hostsConfig` 后注册 `terminal.targets` / `terminal.target_info` / `terminal.verify_target`，并将 MCP smoke tools/list 更新为 25。
3. V2-4：继续保持 `ssh-tmux` Provider 与真实远程 tmux 契约验证同步。
4. V2-5：更新 README/SKILL remote 章节、remote troubleshooting/examples 与 integration evidence。

### V2-4 实现记录（2026-06-13） ✅
...
- `StartInput.target` 已作为可选字段接入 provider 层；SessionManager 的远程 target 全链路接线仍属于后续 V2 工具集成任务，本次不修改 `src/mcp-server.ts`。

#### 下一步待办

1. V2-5: 补充 remote examples、troubleshooting、真实远程/fixture 联调证据与 self-critique。
2. 如提供 Docker SSH fixture，再启用 `ssh-tmux-contract.test.ts` 的真实 start/snapshot/type/attach/kill 合约。

### V2 target tools MCP 接线记录（2026-06-13） ✅
...
| `tests/mcp/mcp-tools.test.ts` | ✅ 更新 | `tools/list` 断言 25 tools；预期 tool 名称加入 3 个 V2 target tools；health 断言 5 个 provider |
...
- `terminal.verify_target` 当前仍是本地 preflight，不建立真实 SSH 连接；真实远程连通性与 tmux/cwd 探测留给 V2-5 fixture/联调证据。

#### 下一步待办

1. ~~V2-5: 补充 remote examples、troubleshooting、README/SKILL 远程章节与 integration evidence。~~ ✅ V2-5 已完成
2. 如提供 Docker SSH fixture，再启用 `ssh-pty-contract.test.ts` / `ssh-tmux-contract.test.ts` 的真实远程合约测试。
3. 运行 `bash .config/setup-e2e-ssh.sh` 初始化 localhost SSH 自测环境，然后 agent 即可使用 `terminal.start({ target: { kind: "ssh", profile: "localhost" } })` 进行 V2 E2E 测试。
...
`.config/hosts.json` 定义了 `localhost` profile — SSH 到本机用于 V2 远程 E2E 测试。
...
- [ ] V1 本地: agent 能启动 python3 REPL，执行代码，正确读取输出
- [ ] V1 本地: agent 能启动 lazygit，导航，安全退出
- [ ] V2 ssh-pty: agent 能 SSH 到 localhost，启动远程 python3，交互正确
- [ ] V2 ssh-tmux: agent 能 SSH 到 localhost，创建远程 tmux session
- [ ] V2 安全: 远程 snapshot 包含 `observationTrust: "untrusted"`
- [ ] V2 安全: 远程 session CWD 违规返回 `REMOTE_CWD_DENIED`
- [ ] V2 安全: 未配置 target 的 inline SSH 被拒绝
