...
import { DependencyMissingError, ProcessExitedError, SessionNotFoundError, SessionTimeoutError, } from "../terminal/errors.js";
...
export class TmuxProvider {
    ...
    async snapshot(sessionId, mode) {
        ...
        const paneHistoryLineCount = snapshotMode === "viewport"
            ...
            : undefined;
        const highlights = tracked.xtermAdapter.detectHighlights(snapshotMode);
    ...
    async waitStable(sessionId, options) {
        ...
        throw new SessionTimeoutError(sessionId, options.timeoutMs, lastSnapshot === null ? "等待稳定超时，且未取得快照" : "等待屏幕稳定超时");
    }
    ...
    createExternalListSession(entry) {
        return {
