 ...
 * 文件默认值也没有 → 使用兜底值
 */
function mergeCsvWithFileDefault(envValue, fileValue, fallback) {
...
export function loadConfig(overrides) {
    ...
    // 分层合并：文件默认值 → 环境变量覆盖
    const enabledProviders = parseEnabledProviders(env.TERMINAL_USE_PROVIDERS, local?.providers);
    const config = {
        ...
        riskyCommandMode: env.TERMINAL_USE_RISKY_COMMAND_MODE ?? local?.riskyCommandMode ?? "deny",
        ...
        defaultProvider: env.TERMINAL_USE_DEFAULT_PROVIDER ?? "native-pty",
 ...
 * 无效名称会被过滤并输出 warn 日志
 */
function parseEnabledProviders(envValue, fileValue) {
    const raw = envValue?.trim() ?? "";
    if (raw.length > 0) {
        return splitCsv(raw).filter((name) => {
            if (ALL_PROVIDER_NAMES.includes(name))
                return true;
