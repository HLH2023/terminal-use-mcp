...
const KEYSCAN_KEY_TYPES = ["ssh-ed25519", "ecdsa-sha2-nistp256", "rsa"];
/**
 ...
 * 7. 不匹配 → 清理临时文件，返回失败
 */
export async function verifyPinnedFingerprintViaKeyscan(profile) {
 ...
 */
export async function verifyPinnedFingerprintOrThrow(profile) {
...
export function cleanupTempKnownHosts(path) {
 ...
 */
async function runSshKeyscan(host, port) {
    const args = [
        "-p", String(port),
 ...
 */
function parseKeyscanOutput(output, host, port) {
    return output
        .split("\n")
        .map((line) => line.trim())
        .filter((line) => {
 ...
 */
function hostMatchesKeyscanLine(hostPart, host, port) {
    // 直接匹配
    if (hostPart === host)
        return true;
    // 带 port 的格式 [<host>]:<port>
    if (hostPart === `[${host}]:${port}`)
        return true;
    // 逗号分隔的 host,ip 格式
 ...
 */
function writeTempKnownHosts(keyLines) {
 ...
 */
async function computeFingerprints(tempKnownHostsPath) {
    return new Promise((resolve) => {
 ...
 */
function extractFingerprintFromKeygenLine(line) {
    if (line.length === 0)
        return undefined;
    // SHA256:xxx 格式
