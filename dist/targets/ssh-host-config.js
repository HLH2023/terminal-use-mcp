...
export function clearHostsConfigCache() {
 ...
 */
export function getHostsConfigPath(env = process.env) {
 ...
 */
export async function loadHostsConfig(configPath, env = process.env) {
 ...
 */
async function loadLegacyHostsJson(filePath, env) {
    let raw;
    try {
 ...
 */
async function loadXdgConfigDir(configDir, env) {
 ...
 */
async function loadProfileOverlays(profilesDir, env) {
 ...
 */
function overlayToProfile(name, overlay) {
 ...
 */
function parseAuthFromOverlay(authValue, profileName) {
 ...
 */
function mergeSshConfigProfiles(profiles, sshConfig) {
    const result = new Map();
...
function applyKnownHostsDefault(profiles) {
...
// ── 旧格式解析（保留向后兼容） ───────────────────────────────
function parseHostsConfig(value, sourcePath) {
...
function extractProfileEntries(value, sourcePath) {
...
function validateLegacyProfile(value, fallbackName, sourcePath) {
    if (!isRecord(value)) {
...
function cloneProfileMap(input) {
