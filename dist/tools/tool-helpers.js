...
import { parseKeyExpr } from "../terminal/keymap.js";
import { DependencyMissingError, InternalError, ProcessExitedError, ProviderCapabilityUnsupportedError, ProviderNotAvailableError, RemoteTmuxNotAvailableError, SessionNotFoundError, TerminalUseError, } from "../terminal/errors.js";
...
export function sessionToPublicInfo(session) {
    return {
...
export function okToolResult(summary, structuredContent) {
    return {
...
export class ProviderExecutor {
    ...
    providers;
    hostsConfig;
    constructor(sm, providers, hostsConfig) {
        this.sm = sm;
        this.providers = providers;
        this.hostsConfig = hostsConfig;
    }
    async executeTmuxList(input = {}) {
        ...
        const managedNames = new Set(this.sm.listSessions()
    ...
    async executeTmuxKillPreview(name, input = {}) {
    ...
    async executeTmuxKill(name, input = {}) {
        const tmuxName = name.trim();
        if (tmuxName.length === 0) {
            throw new TerminalUseError({
                code: "INTERNAL_ERROR",
    ...
    async executeSnapshot(sessionId, mode = "viewport") {
    ...
    async executeWaitForText(sessionId, text, options) {
        const session = this.sm.getSession(sessionId);
        const provider = this.getProvider(session.providerName);
    ...
    async executeWaitStable(sessionId, options) {
        const session = this.sm.getSession(sessionId);
        const provider = this.getProvider(session.providerName);
    ...
    async executeFind(sessionId, pattern, regex, includeScrollback) {
        const session = this.sm.getSession(sessionId);
        const provider = this.getProvider(session.providerName);
    ...
    async executeScroll(sessionId, direction, lines) {
        const session = this.sm.getSession(sessionId);
        const provider = this.getProvider(session.providerName);
    ...
    async executeMouseClick(sessionId, input) {
    ...
    async executeMouseScroll(sessionId, input, lines) {
        const session = this.getLiveInputSession(sessionId);
        const provider = this.getProvider(session.providerName);
    ...
    async executeType(sessionId, text) {
        const session = this.getLiveInputSession(sessionId);
        const provider = this.getProvider(session.providerName);
        await session.queue.enqueue(async () => {
    ...
    async executePress(sessionId, keyExpr, parsed) {
        const session = this.getLiveInputSession(sessionId);
        const provider = this.getProvider(session.providerName);
        await session.queue.enqueue(async () => {
    ...
    async executePaste(sessionId, text, mode) {
        const session = this.getLiveInputSession(sessionId);
        const provider = this.getProvider(session.providerName);
        await session.queue.enqueue(async () => {
    ...
    async executeResize(sessionId, cols, rows) {
        const session = this.sm.getSession(sessionId);
        const provider = this.getProvider(session.providerName);
    ...
    getEvents(sessionId, limit, sinceSeq) {
        const session = this.sm.getSession(sessionId);
     ...
     */
    async executeSendSignal(sessionId, signal) {
        const session = this.sm.getSession(sessionId);
        const provider = this.getProvider(session.providerName);
        await session.queue.enqueue(async () => {
            if (signal === "SIGINT") {
    ...
    async resolveTmuxToolTarget(input) {
        ...
        const target = profile === undefined ? input.target : { kind: "ssh", profile };
    ...
    async killLocalTmuxSession(name) {
        await this.assertProviderAvailable("tmux");
    ...
    async killRemoteTmuxSession(target, name) {
        await this.assertProviderAvailable("ssh-tmux");
    ...
    assertCapability(provider, capability, action) {
...
function execLocalTmuxCommand(args) {
...
function parseTmuxListSessionsOutput(stdout) {
    return stdout
        .split("\n")
        .map((line) => line.trim())
...
function createTmuxCommandError(provider, action, sessionName, result) {
