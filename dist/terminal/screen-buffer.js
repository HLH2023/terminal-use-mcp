 ...
 */
export function screenBufferToString(buffer) {
    let lastTextRow = -1;
 ...
 * 与“内容从第 0 行开始”。
 */
export function getContentRange(buffer) {
    let firstRow = -1;
    let lastRow = -1;
 ...
 * 标记为空，光标固定在左上角。
 */
export function createEmptyScreenBuffer(sessionId, cols, rows) {
    return {
        sessionId,
 ...
 * 被用于安全决策。
 */
export function isFullscreenHeuristic(buffer) {
