...
/** 计算 SGR 修饰符值 */
function computeModifier(modifiers) {
    let value = 0;
 ...
 */
function applyModifierToSequence(baseSeq, modifier) {
 ...
 */
export function parseKeyExpr(expr) {
...
function validateAndNormalize(rawExpr, rawModifiers, rawKey) {
    // 校验修饰键名称
 ...
 *  5. 无修饰 + 单字母     → 原样字符
 */
export function parsedKeyToAnsiSequence(parsed) {
    ...
    if (modifiers.length === 0) {
        ...
        if (FN_KEY_MAP[key] !== undefined)
            return FN_KEY_MAP[key];
    }
    // 修饰键 + 命名/功能键 → SGR 修饰编码
    ...
    // 兜底: 不应该到这里
    throw new InvalidKeyExprError(`modifiers=${modifiers.join("+")} + key=${key}`, "unable to encode key expression");
}
// ============================================================
 ...
 * @throws InvalidKeyExprError 无法解析时
 */
export function keyExprToAnsiSequence(expr) {
 ...
 */
export function parsedKeyToTmuxKey(parsed) {
    const { modifiers, key } = parsed;
    // 构建修饰键前缀
    let prefix = "";
    ...
    // 不应到达
    throw new InvalidKeyExprError(`modifiers=${modifiers.join("+")} + key=${key}`, "unable to map to tmux key name");
}
/** 便捷入口: 按键字符串 → tmux 键名 */
export function keyExprToTmuxKey(expr) {
 ...
 */
export function parsedKeyToTuiUseKey(parsed) {
    const { modifiers, key } = parsed;
    // 单字母
    if (/^[a-z]$/.test(key)) {
        if (modifiers.length > 0) {
    ...
    throw new InvalidKeyExprError(`modifiers=${modifiers.join("+")} + key=${key}`, "unable to map to tui-use key name");
}
/** 便捷入口: 按键字符串 → tui-use 键名 */
export function keyExprToTuiUseKey(expr) {
 ...
 */
export function keyToAnsiSequence(key) {
 ...
 */
export function keyToTmuxKey(key) {
 ...
 */
export function keyToTuiUseKey(key) {
