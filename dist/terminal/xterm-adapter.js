...
export class XtermAdapter {
    ...
    constructor(cols, rows, scrollback) {
        ...
        this.disposables.push(this.terminal.onTitleChange((newTitle) => {
            this.title = newTitle;
        }));
    ...
    detectHighlights(mode = "viewport") {
        if (this.disposed) {
            throw new TerminalUseError({
                code: "INTERNAL_ERROR",
    ...
    getScreenReadRange(buffer, mode) {
        if (mode === "full") {
    ...
    countScrollbackLines(buffer) {
        return Math.max(0, buffer.length - this.terminal.rows);
    }
    /** 将 xterm cell 属性映射为工具层高亮类别。 */
    detectCellHighlightKind(isInverse, isBold, fgColor) {
        if (isInverse !== 0) {
            return "inverse";
        }
    ...
    flushHighlightSpan(highlights, line, row, span) {
        if (!span) {
            return undefined;
        }
        highlights.push({
