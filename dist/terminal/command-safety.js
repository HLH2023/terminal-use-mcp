...
const WRAPPER_OPTION_SIGNATURES = {
    timeout: [
 ...
 */
function stripWrapperArgumentsArgv(wrapper, tokens) {
 ...
 */
export function extractBaseCommandArgv(argv) {
    let tokens = [...argv];
...
function basenameCrossPlatform(pathValue) {
 ...
 */
function isSubdirectoryCanonical(childCanonical, parentCanonical) {
 ...
 */
export function maybeWrapWithShell(input) {
 ...
 */
export function isCommandSafeArgv(command, args, allowedCommands = [], deniedCommands = [], riskyMode = "deny") {
 ...
 */
export function isCommandSafe(command, allowedCommands = [], deniedCommands = [], riskyMode = "deny") {
...
function buildAllowedSet(allowedCommands) {
    return new Set(allowedCommands.map((command) => command.toLowerCase()));
}
function buildDeniedSet(deniedCommands) {
 ...
 * 绝不 fallback 到字符串比较。
 */
export async function isCwdAllowed(cwd, workspaceRoot = process.cwd(), allowedCwdRoots = []) {
