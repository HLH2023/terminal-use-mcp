 ...
 */
const SGR_BUTTON_CODE = {
    left: 0,
    middle: 1,
    right: 2,
};
const SGR_SCROLL_CODE = {
    up: 64,
    down: 65,
};
/**
 ...
 */
function computeSgrModifierMask(event) {
    let mask = 0;
    if (event.shift)
        mask |= 4;
    if (event.alt)
 ...
 */
export function mouseClickToSgrSequence(event) {
 ...
 * 滚轮事件只有按下 (M), 没有释放 (m)
 */
export function mouseScrollToSgrSequence(event) {
    assertMouseCoordsAtLeastOne(event.col, event.row);
 ...
 */
export function mouseClickToX10Sequence(event) {
    assertMouseCoordsAtLeastOne(event.col, event.row);
...
export function mouseScrollToX10Sequence(event) {
    assertMouseCoordsAtLeastOne(event.col, event.row);
    assertMouseCoordsWithinX10Limit(event.col, event.row);
 ...
 */
export function mouseClickToFullSgrSequence(event) {
 ...
 */
export function mouseClickToTmuxSequence(event) {
    // tmux 内部使用 SGR-1006 格式转发，同 ANSI SGR
    return mouseClickToFullSgrSequence(event);
}
export function mouseScrollToTmuxSequence(event) {
...
export function validateMouseCoords(col, row, maxCols, maxRows) {
    if (col < 1 || row < 1) {
...
function assertMouseCoordsAtLeastOne(col, row) {
    if (col < 1 || row < 1) {
        throw new InvalidMouseCoordsError(col, row, "坐标必须 >= 1 (1-based)");
    }
}
function assertMouseCoordsWithinX10Limit(col, row) {
    if (col > 227 || row > 227) {
