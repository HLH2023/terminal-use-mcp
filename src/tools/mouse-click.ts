...
export function registerMouseClickTool(server: McpServer, executor: ProviderExecutor, logger: Logger): void {
  server.registerTool(
    "terminal.mouse_click",
    {
      description:
        "Click the mouse at a specific position in the terminal. " +
    ...
    async (input) => {
      try {
        ...
        const output: MouseClickToolOutput = { ok: true, button, col: input.col, row: input.row }
