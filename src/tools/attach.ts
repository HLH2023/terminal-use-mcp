...
export function registerAttachTool(server: McpServer, sm: SessionManager, logger: Logger): void {
  server.registerTool(
    "terminal.attach",
    {
    ...
    async (input) => {
      try {
        ...
        const session = await sm.attach(target, input.provider as ProviderName | undefined)
