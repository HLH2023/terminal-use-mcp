 ...
 * 仅展开字符串值，对象和数组递归处理。
 */
export function expandEnvVars<T>(value: T, env: NodeJS.ProcessEnv = process.env): T {
  if (typeof value === "string") {
    return expandStringEnvVars(value, env) as T
  }
  if (Array.isArray(value)) {
    return value.map((item) => expandEnvVars(item, env)) as T
  }
...
function expandStringEnvVars(value: string, env: NodeJS.ProcessEnv): string {
  return value.replace(ENV_VAR_PLACEHOLDER, (match, varName: string) => {
 ...
 * 用于检测配置文件中的意外明文凭据。
 */
function noForbiddenSecretKeys(data: unknown, label: string): boolean {
...
/** 展开路径中的 ~ 为 os.homedir() */
export function expandTildeInPath(value: string): string {
...
/** 递归展开对象中所有路径值的 ~ */
export function expandTildeInObject<T>(value: T): T {
  if (typeof value === "string") {
    return expandTildeInPath(value) as T
  }
  if (Array.isArray(value)) {
    return value.map((item) => expandTildeInObject(item)) as T
  }
  if (value !== null && typeof value === "object") {
    const result: Record<string, unknown> = {}
