...
export type FingerprintVerifyResult =
  | { ok: true; matches: true; algorithm: string; fingerprint: string }
  | { ok: true; matches: false; algorithm: string; expectedFingerprint: string; actualFingerprint: string }
  | { ok: false; reason: "no_fingerprint_provided" | "fingerprint_format_invalid" | "key_unavailable"; detail: string }
 ...
 */
export function computeHostFingerprint(publicKey: string, algorithm: "sha256" | "md5"): string {
...
function formatParsedFingerprint(fingerprint: ParsedFingerprint): string {
