...
tools/local/terminal-use-mcp/
  ...
  src/
    ...
    providers/
      ...
      tmux-provider.ts            # tmux 3.2+ backend
...
## 5. MCP Tools — 22 local + 3 remote (see §26.5)
...
- tools/list 包含 25 tools (22 local + 3 remote target tools，参见 §5 和 §26.5)
- terminal.health 可调用
...
|------|------|------|-----------|
| 0 | 项目脚手架 | - | 5 |
| 1 | 核心类型 + Safety 层 | 阶段 0 | 9 |
| 2 | Terminal 层 (xterm/screen/wait/transcript) | 阶段 1 | 5 |
| 3 | SessionManager + PromiseQueue | 阶段 1 | 1 |
| 4 | NativePtyProvider | 阶段 2+3 | 1 |
| 5 | TmuxProvider | 阶段 1+3 | 1 |
| 6 | MCP Tools (22) | 阶段 3-5 | 22 |
| 8 | MCP Resources + Prompts | 阶段 7 | 4 |
| 9 | MCP Server 入口 | 阶段 7+8 | 2 |
| 10 | Unit tests | 阶段 1-3 | 6 |
| 11 | Test fixtures | - | 6 |
| 12 | Provider contract + MCP smoke tests | 阶段 9+11 | 4 |
| 13 | SKILL.md | 阶段 9 | 1 |
| 14 | Examples | 阶段 9 | 7 |
| 15 | 验证 + 最终报告 | All | 0 |
...
| Native Windows | ❌ Unsupported (current) | ConPTY 支持列为后续工作; tmux 不可用; PowerShell 交互不受支持 |

**Native Windows ConPTY 路径**: 通过 `node-pty` 的 ConPTY 后端实现，配合 Windows Terminal 渲染。不依赖 tmux。
...
4. Native Windows 不支持 (ConPTY 实现列为后续工作，参见 §18)
...
## 21. Remote Terminal Design Overview

Remote Terminal Objectives: **Local + Remote Terminal Computer Use over MCP**。

Building on local terminal control, remote SSH terminal control lets agents interactively control programs on remote hosts via MCP.

### 21.1 Remote Architecture Overview
...
Two remote providers are added:
...
Remote introduces a core distinction:
...
```ts
type ProviderName =
  | "native-pty"    // Local PTY
  | "tmux"          // Local tmux
  | "ssh-pty"       // Remote SSH PTY channel
  | "ssh-tmux"      // Remote SSH tmux session
```

### 21.4 Remote terminal control does not replace ACP

ACP 是结构化 agent 协议; 本工具是 terminal computer use，用于操作已有终端界面、远程联调、排障、无 API 的 TUI 程序和外部 CLI agent。

---

## 22. Remote Core Types
...
export type TerminalTarget =
  ...
  | {
      ...
      username?: string
      auth?: SshAuthRef
      knownHostPolicy?: "strict"  // only strict is supported
    }
```
...
```ts
{ type: "password" }  // Password login is not supported
```
...
Adding to the local base `target?` 字段:

```ts
export type StartTerminalInput = {
  provider?: ProviderName | "auto"   // ssh-pty or ssh-tmux
  target?: TerminalTarget            // remote target
  command: string
  args?: string[]
  cwd?: string
  cols?: number
...
## 24. ssh-pty Provider Specification

### 24.1 定位

`ssh-pty` 是 primary remote provider，通过 SSH 建立远程 PTY channel，reusing the local xterm adapter、snapshot、wait、transcript、riskSignals、redaction 体系。
...
channel data → xtermAdapter.write()
  ↓
snapshot / wait / transcript (reuses local system)
```
...
| reconnect | Auto-reconnect is not required，但断线状态必须正确检测和标记 |
...
| paste | bracketed / line-by-line / raw | same as local paste modes |
| resize | SSH window-change request | `channel.setWindow(rows, cols)` |
| kill | `channel.close()` + `client.end()` | 先关 channel，再断 client |
| snapshot | xterm buffer | reuses the local snapshot system |
| wait_for_text | xterm snapshot 轮询 | reuses the local wait system |
| wait_stable | channel data dirty flag debounce | reuses the local wait system |
| export_transcript | 本地 artifact | reuses the local transcript system |
...
## 25. ssh-tmux Provider Specification

### 25.1 定位

`ssh-tmux` 是 persistent remote session provider，用于远程长时间运行、断线后恢复、人类可 attach、agent 可观察和控制远程 tmux pane。
...
在 Adding 3 remote target tools to the 22 local tools，总计 **25 个**。
...
All session operations tools (snapshot/type/press/paste/wait/kill/export_transcript 等) 自动支持远程 session，无需额外参数。Session 创建时已绑定 target，后续操作按 sessionId 路由即可。

### 26.5 Tools 总数

| Category | Local | Remote (new) | Total |
|------|-----|---------|------|
| Session lifecycle | 7 | 0 | 7 |
| Observation | 5 | 0 | 5 |
| Input | 3 | 0 | 3 |
| Meta | 7 | 0 | 7 |
| Target (New category) | 0 | 3 | 3 |
| **合计** | **22** | **3** | **25** |

---

## 27. Remote Provider Auto-Selection Rules
...
Same as local: native-pty first，不可用时 fallback。
...
## 28. Remote Safety Layer Extensions
...
Remote error codes added:

```ts
type TerminalUseErrorCode =
  | // Existing local error codes
    "SESSION_NOT_FOUND"
  | "PROVIDER_NOT_AVAILABLE"
  | "PROVIDER_CAPABILITY_UNSUPPORTED"
  | "SESSION_TIMEOUT"
...
```ts
type: 
  | "confirmation_prompt"           // Local: 确认提示
  | "credential_prompt"             // Local: 凭据提示
  | "destructive_prompt"            // Local: 破坏性提示
  | "external_agent_permission"     // Local: 外部 agent 权限请求
  | "remote_privilege_prompt"       // Remote: 远程特权提示 (sudo/su 等)
  | "remote_host_key_prompt"        // Remote: 远程 host key 确认提示
```
...
除 Existing local secret redaction 外，远程 transcript 额外脱敏:
...
## 30. Remote File Structure

在 Adding to the existing local directory structure以下文件:
...
| 阶段 | 内容 | 依赖 | 预计文件数 |
|------|------|------|-----------|
| Remote-0 | 远程设计落文档 (DEV-PLAN / PROGRESS / README / SKILL.md) | Local complete | 4 |
| Remote-1 | TerminalTarget + SshHostProfile + hosts.json loader + RemoteCwdPolicy + tests | Remote-0 | 7 |
| Remote-2 | known_hosts / pinned fingerprint + ssh-agent auth + key-file + verify_target + tests | Remote-1 | 5 |
| Remote-3 | ssh-pty Provider | Remote-2 | 3 |
| Remote-4 | ssh-tmux Provider | Remote-2 | 3 |
| Remote-5 | Remote examples + troubleshooting + integration evidence + self-critique | Remote-3 + Remote-4 | 8 |

总 Remote phase is expected to add文件: ~30 个 (含 tests + fixtures)。

---

## 32. Remote completion标准

Remote completion必须满足以下 28 项标准:
